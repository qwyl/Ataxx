...
class Board {
    ...
    Board(Board board0) {
        ...
        _currentJumps = new ArrayList<Integer>();
    ...
    void makeMove(Move move) {
        ...
        if (move.isJump()) {
            if (_numJumps == 0
                    || _allMoves.get(allMoves().size() - 1).isJump()) {
                _numJumps++;
            }
        ...
        _currentJumps.add(_numJumps);
        set(move.toIndex(), _whoseMove);
        _numPieces[_whoseMove.ordinal()]++;

        for (int c = -1; c < 2; c++) {
            for (int r = -1; r < 2; r++) {
                char checkC = (char) (move.col1() + c);
                char checkR = (char) (move.row1() + r);
                if (get(checkC, checkR) == opponent) {
                    set(checkC, checkR, _whoseMove);
                    _numPieces[_whoseMove.ordinal()]++;
        ...
        if (_numJumps >= JUMP_LIMIT || open == 0
    ...
    void pass() {
        ...
        assert _numPieces[_whoseMove.ordinal()] >= 1;
        startUndo();
    ...
    void undo() {
        if (allMoves().size() > 1) {
            _numJumps = _currentJumps.get(_currentJumps.size() - 2);
            _currentJumps.remove(_currentJumps.size() - 1);
        }
        _whoseMove = _whoseMove.opposite();
        _allMoves.remove(_allMoves.size() - 1);
        while (_undoSquares.peek() != null) {
            int currentSquare = _undoSquares.pop();
            PieceColor current = _undoPieces.pop();
            if (get(currentSquare) == RED) {
                _numPieces[RED.ordinal()]--;
            } else if (get(currentSquare) == BLUE) {
                _numPieces[BLUE.ordinal()]--;
            }
            if (current == RED) {
                _numPieces[RED.ordinal()]++;
            ...
            unrecordedSet(currentSquare, current);
        }
        _undoSquares.pop();
        int open = 0;
        for (int i = 0; i < _board.length; i++) {
    ...
    void setBlock(char c, char r) {
        ...
        _totalOpen--;
        if (legalBlock((char) (Math.abs(('d' - c)) + 'd'), r)) {
            unrecordedSet((char) (Math.abs(('d' - c)) + 'd'), r, BLOCKED);
            _totalOpen--;
        }

        if (legalBlock(c, (char) ('4' + Math.abs('4' - r)))) {
            unrecordedSet(c, (char) ('4' + Math.abs('4' - r)), BLOCKED);
            _totalOpen--;
        }
        if (legalBlock((char) (Math.abs(('d' - c)) + 'd'),
                (char) ('4' + Math.abs('4' - r))))  {
            unrecordedSet((char) (Math.abs(('d' - c)) + 'd'),
                    (char) ('4' + Math.abs('4' - r)), BLOCKED);
            _totalOpen--;
        }
     ...
     * keep track jumps.
     */
    private ArrayList<Integer> _currentJumps;
